        ...
        <div className="space-y-6">
            ...
            {settings.stats && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                    ...
                    <StatCard icon="fa-building" title={t('housing.buildings')} value={data.stats.totalBuildings} gradient="bg-gradient-to-br from-green-400 to-green-600" onClick={() => navigate('/housing')} />
                    <StatCard icon="fa-door-closed" title={t('housing.tabs.rooms')} value={data.stats.totalRooms} gradient="bg-gradient-to-br from-indigo-400 to-purple-600" onClick={() => navigate('/housing')} />
                    <StatCard icon="fa-users-cog" title={t('dashboard.admin.totalUsers')} value={data.users.length} gradient="bg-gradient-to-br from-purple-400 to-pink-600" onClick={() => navigate('/users')} />
                    <StatCard icon="fa-history" title={t('dashboard.admin.totalLogs')} value={data.activityLogs.length} gradient="bg-gradient-to-br from-slate-400 to-slate-600" onClick={() => navigate('/activity-log')} />
                </div>
            )}
