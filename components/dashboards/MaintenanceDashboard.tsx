        ...
        <div className="space-y-6">
            {settings.stats && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <StatCard icon="fa-folder-open" title={t('dashboard.maintenance.newTickets')} value={openRequests.length} color="bg-red-500" onClick={() => navigate('/maintenance')} />
                    <StatCard icon="fa-tasks" title={t('dashboard.maintenance.inProgress')} value={inProgressRequests.length} color="bg-yellow-500" onClick={() => navigate('/maintenance')} />
                    <StatCard icon="fa-check-circle" title={t('dashboard.maintenance.resolved')} value={data.maintenanceRequests.filter(r=>r.status === 'resolved').length} color="bg-green-500" onClick={() => navigate('/maintenance')} />
                </div>
            )}
